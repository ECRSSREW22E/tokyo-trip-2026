  ...
  mount.innerHTML = `
    <div class="spot-page">
      ...
      <section class="spot-hero">
        <figure class="spot-visual" style="position:relative;margin:0;min-width:0"><img src="${imageUrl(d.image)}" alt="${d.title}">${credit}</figure>
      ...
      <div class="shell spot-body">
        <main>
          ...
          <section class="spot-gallery" aria-labelledby="gallery-heading">
            ...
            <div class="gallery-stage"><img id="gallery-main" src="${imageUrl(gallery[0][0])}" alt="${gallery[0][1]}"><div class="gallery-caption"><strong id="gallery-title">${gallery[0][1]}</strong><span id="gallery-description">${gallery[0][2]}</span><small><span id="gallery-source">${gallery[0][3]}</span> ・ <a href="${root}../images/spots/SOURCES.md" style="color:#fff">照片出處</a></small></div></div>
        ...
        <aside class="panel source-panel"><div class="eyebrow">SOURCE ADMIN</div><h2>資料管理</h2><p>${d.sources.length} 個正式來源<br>${social.length} 則公開社群實訪</p><details class="source-details"><summary>展開本頁正式來源</summary><ol class="source-list">${d.sources.map((source) => `<li><a href="${source[1]}" target="_blank" rel="noopener">${source[0]}</a><small>${source[2]}</small></li>`).join('')}</ol></details><div class="actions"><a class="btn" href="${root}sources/index.html">完整來源中心</a><a class="btn alt" href="${root}destinations/index.html">景點目錄</a></div></aside>
      </div>
    </div>`;
